<template>
    <div class="q-pa-md setting-tree">
        <q-tree ref="tree" :dense="isPhone" no-selection-unset text-color="text" color="text" :nodes="simple"
            node-key="label" selected-color="active" v-model:selected="innerSelected" default-expand-all>
            <template v-slot:default-header="prop">
                <div class="row items-center q-ma-xs">
                    <q-icon :name="prop.node.icon || 'share'" :size="prop.node.iconSize" class="q-mr-sm" />
                    <div class="text-weight-bold" :style="`font-size: ${prop.node.fontSize}`">{{ prop.node.label }}
                    </div>
                </div>
            </template>
        </q-tree>
